...
export default class ReminderApp {
  ...
  private handleToggleCompletion(): void {
    if(this._remindersHandler.size() == 0) {
      ...
      const number = this.getUserChoice("reminder number to edit", true);
      this._remindersHandler.toggleCompletion(parseInt(number));
  ...
  private handleModifyReminders(): void {
    ...
    /*
            ...
            optional for now( 5 ) If user wishes to also toggle reminder status, do so (this._remindersHandler.toggleCompletion)...feeding in ( 2 )
            ( 6 ) Logger.log('\n  🏁   Reminder Modified');
        */
  }
  ...
  private handleAddReminder(): void {
    const reminderDescription = this.getUserChoice("reminder", false);
    const reminderTag = this.getUserChoice("reminder tag", false);
